using Oceananigans, StructArrays, Printf, JLD2, Statistics, Adapt, CUDA
...
Lx, Ly, Lz = 32, 4, 8
Nx, Ny, Nz = 8 .*(Lx, Ly, Lz)
grid = RectilinearGrid(Oceananigans.GPU();size=(Nx, Ny, Nz), extent=(Lx, Ly, Lz), topology=(Periodic, Periodic, Bounded))
...
particle_struct = StructArray{GiantKelp}((CuArray([12.0]), 
					  CuArray([2.0]), 
					  CuArray([-8.0]), 
					  CuArray([12.0]), 
					  CuArray([2.0]),
...
@inline function guassian_smoothing(r, z, rᵉ)
    if z>0
        r = sqrt(r^2 + z^2)
...
@inline u_forcing_func(args...) = 1e-5
